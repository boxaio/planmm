"""
MediaPipe Face Landmarker（478 点）+ 仓库内 ``mesh_render.render_mesh``：
...
DEFAULT_MESH_FILES: tuple[str, ...] = (
    "bfm_template.obj",
    "bfm_1066_neutral.obj",
    "bfm_1001_neutral.obj",
    "bfm_1066_expr.obj",
)
...
def fuse_hits_prioritize_reference_view_then_median_reference_frames(
    ...
    """
    ``hits_per_view`` (K, L, 3)。优先用 ``ref_view_index`` 视图上的击中点（与 ``bfm_template`` 几何对齐）；
...
def save_debug_lmks2d_overlays(
    ...
    *,
    ...
    print(f"[bfm_lmks478] wrote {len(written)} overlay images under {out_dir}")
    return written
...
def save_debug_reproj_compare_overlays(
    ...
    *,
    ...
    for ki in range(len(rgb_images)):
        ...
        cv2.imwrite(str(out_p), vis_bgr)
        written.append(out_p)
    print(f"[bfm_lmks478] wrote {len(written)} reproj_compare images under {out_dir}")
    return written
...
def compute_lmks478_barycentric_template(
    *,
    ...
    print(f"[bfm_lmks478] R_max={R_max:.6f}, d={cam_d:.6f}, zn/zf={znear:.6f}/{zfar:.6f}")
    ...
    for vi in range(K_views):
        ...
        if np.any(ok_mask):
            ...
            print(
                f"[bfm_lmks478] reproj RMS view={variants[vi][0]}: "
            ...
            print(f"[bfm_lmks478] reproj RMS view={variants[vi][0]}: no finite errors")
    ...
    if np.any(np.isfinite(e_bary_repr)):
        print(
            f"[bfm_lmks478] after closest+bary, template-view reproj: "
...
def main_cli() -> None:
    parser = argparse.ArgumentParser(description="BFM MediaPipe478 → template barycentric")
    parser.add_argument("--out", type=Path, default=_DATASET_DIR / "bfm_lmks478_bary.pkl")
    parser.add_argument("--res", type=int, default=512)
    parser.add_argument(
        "--debug-overlay-dir",
        type=Path,
        default='/media/ubuntu/SSD/PHACK_code/dataset/bfm_lmks478_debug_overlay',
