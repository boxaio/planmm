...
# region_name -> single fids txt path.
_REGION_FID_TXT: Dict[str, Union[Path, Tuple[Path, ...]]] = {
    'face_0': _DATASET_DIR / 'hack_face_0_fids.txt',
    'face_1': _DATASET_DIR / 'hack_face_1_fids.txt',
    'eye_0': _DATASET_DIR / 'hack_eye_0_fids.txt',
    'eye_1': _DATASET_DIR / 'hack_eye_1_fids.txt',
    'nose': _DATASET_DIR / 'hack_nose_fids.txt',
    'mouth': _DATASET_DIR / 'hack_mouth_fids.txt',
    'forehead': _DATASET_DIR / 'hack_forehead_fids.txt',
    'frown': _DATASET_DIR / 'hack_frown_fids.txt',
    'skull_neck': _DATASET_DIR / 'hack_skull_neck_fids.txt',
    'ear_0': _DATASET_DIR / 'hack_ear_0_fids.txt',
    'ear_1': _DATASET_DIR / 'hack_ear_1_fids.txt',
    'eye_0_inner': _DATASET_DIR / 'hack_eye_0_inner_fids.txt',
    'eye_1_inner': _DATASET_DIR / 'hack_eye_1_inner_fids.txt',
    'nose_inner': _DATASET_DIR / 'hack_nose_inner_fids.txt',
    'mouth_inner': _DATASET_DIR / 'hack_mouth_inner_fids.txt',
}


def load_hack_seg_fids_from_txt(txt_path: Union[str, Path]) -> List[int]:
    p = Path(txt_path)
...
def _collect_fids_for_region(name: str) -> np.ndarray:
    ...
    if isinstance(src, tuple):
        merged: List[int] = []
        for txt in src:
            merged.extend(load_hack_seg_fids_from_txt(txt))
        fids = np.asarray(sorted(set(merged)), dtype=np.int64)
    else:
        fids = np.asarray(load_hack_seg_fids_from_txt(src), dtype=np.int64)
    return fids
...
def save_neck_split_patch_pkl(
    reference_obj: Union[str, Path] = _DATASET_DIR / 'hack_template.obj',
...
def save_region_info_pkl(
    ...
    """Write ``hack_region_info.pkl`` with per-region ``fids``, ``vids``, ``boundary_vids``, and ``patch_faces``."""
...
if __name__ == '__main__':
    ...
    show_hack_regions_polyscope(region_info=region_info, boundary_vertices_gui=True)
