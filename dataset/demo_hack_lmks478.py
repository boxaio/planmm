"""
在 Polyscope 中显示 HACK 模板网格及 ``hack_lmks478.py`` 生成的 478 个 3D 关键点。

默认读取 ``dataset/hack_lmks478_bary.pkl``；重心坐标减去模板质心后与 ``compute_lmks478_barycentric_template`` 中一致，
...
DEFAULT_PKL = _DATASET_DIR / "hack_lmks478_bary.pkl"

mouth_open_mesh_path = '/home/ubuntu/MyExp/TRACK/data/hack_data/hack_template_turn_left_open_mouth.obj'
    ...
    *,
    ...
    if not pkl_path.is_file():
        raise FileNotFoundError(
            f"未找到关键点文件：{pkl_path}\n"
            "请先运行：python dataset/hack_lmks478.py --out <path.pkl>"
        )
    ...
    tmpl_path = Path(mouth_open_mesh_path)
    ...
    ps.init()
    ps.set_program_name("hack_lmks478")
    ...
    print(f"[demo_hack_lmks478] mesh {tmpl_path} | V={verts.shape[0]} F={faces.shape[0]} | lm={lm_xyz.shape[0]}")
    ps.show()


def main() -> None:
    ap = argparse.ArgumentParser(description="Polyscope：HACK 模板 + MediaPipe478 3D 关键点")
    ap.add_argument("--pkl", type=Path, default=DEFAULT_PKL, help="hack_lmks478 输出的 pickle")
    ap.add_argument(
        "--point-radius-rel",
        type=float,
        default=1e-3,
