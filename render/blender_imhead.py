...
for filename in params.input:
    ...
    if params.sequence:
        bpy.data.scenes["Scene"].render.filepath = os.path.join(params.output, 
                                                                f"{'smooth' if params.smooth else 'wireframe'}_{params.it:04d}.png")
    else:
        bpy.data.scenes["Scene"].render.filepath = os.path.join(params.output, 
                                                                f"{name}_{'smooth' if params.smooth else 'wireframe'}.png")
