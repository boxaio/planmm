...
from config.env_paths import VALID_VERTS_UV_MESH, FLAME_UV_COORDS, head_template_ply
...
def uv_pred_to_mesh(output, mask, rgb_img, right_ear=None, left_ear=None):
    valid_verts = np.load(f'{VALID_VERTS_UV_MESH}')

    m_test = trimesh.load(f'{head_template_ply}', process=False)
    edges = m_test.edges_unique

    g = nx.from_edgelist(m_test.edges_unique)
    one_ring = [list(g[i].keys()) for i in range(len(m_test.vertices))]

    can_uv = torch.from_numpy(np.load(f'{FLAME_UV_COORDS}')).cuda().unsqueeze(0).float()
        ...
        for j in one_ring[i]:
            if j in drawn_verts:
                if torch.abs(gt_2_verts[0, i] - gt_2_verts[0, j]).sum(-1) > 35:
                    continue
                draw.line([
                    (gt_2_verts[0, i, 0].int().item(), gt_2_verts[0, i, 1].int().item()),
                    (gt_2_verts[0, j, 0].int().item(), gt_2_verts[0, j, 1].int().item()),
                ], fill=128)
    empty = np.array(im)
    #for i in range(pixel_position_height.shape[1]):
    #    if i not in valid_verts:
