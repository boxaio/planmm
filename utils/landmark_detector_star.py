...
from external.STAR.star.lib import utility
from external.STAR.star.asset import predictor_path, model_path
...
class LandmarkDetectorSTAR:
    ...
    def annotate_landmarks(self, dataloader, output_path):
        """
        ...
        logger.info(f"Saving landmarks to: {output_path}")
        if not output_path.parent.exists():
            output_path.parent.mkdir(parents=True)
        np.savez(output_path, **lmk_dict)
...
#     data_cfg = tyro.cli(DataConfig)
#     dataset = import_module(data_cfg._target)(
#         cfg=data_cfg,
#         img_to_tensor=False,
...
if __name__ == "__main__":
    ...
    data_cfg = tyro.cli(NersembleV2DataConfig)
