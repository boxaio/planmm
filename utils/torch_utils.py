    ...
    if partial_restore:
        ...
        for k, v in saved_net.items():
            if k not in net_dict:
                continue
            if tuple(v.shape) != tuple(net_dict[k].shape):
                skipped_shape.append((k, tuple(v.shape), tuple(net_dict[k].shape)))
                continue
            kept[k] = v
        saved_net = kept
        ...
        if skipped_shape:
            print("parameters skipped (shape mismatch):")
            for name, old_s, new_s in skipped_shape:
                print(f"  {name}: ckpt{old_s} -> model{new_s}")
