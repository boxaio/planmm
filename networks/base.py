...
# class PoissonSolver(torch.autograd.Function):
#     @staticmethod
#     def forward(ctx, solvers: list, rhs: torch.Tensor):
#         # solvers: [B,] cholespy.CholeskySolverF（CPU）
#         # rhs: (B, V, C) GPU float32 Tensor
#         B, V, C = rhs.shape
#         ctx.solvers = solvers
#         ctx.rhs_device = rhs.device
#         ctx.rhs_shape = rhs.shape

#         x = torch.zeros_like(rhs, device=rhs.device, dtype=rhs.dtype)
#         for b in range(B):
#             solver = solvers[b]
#             solver.solve(rhs[b].contiguous(), x[b].contiguous())  
#         return x
...
#         B, V, C = grad_output.shape
#         # 梯度处理逻辑与forward一致（整体CPU求解→GPU返回）
#         grad_output = grad_output.contiguous()
#         f_grad = torch.zeros_like(grad_output)

#         for b in range(B):
#             solver = ctx.solvers[b]
#             solver.solve(grad_output[b].contiguous(), f_grad[b].contiguous())
#         del ctx.solvers
#         return None, f_grad
...
class PoissonBlockMLP(nn.Module):
    def __init__(self, in_c, out_c, width, act_layer=nn.GELU, init_values=1.0, drop=0., drop_path=0.,
        norm=True, grad_inputs=False, extra_feats: int=0, 
    ):
        super().__init__()        
        ...
        in_c = in_c + extra_feats  # concat extra features
...
class MlpBlock(nn.Module):
    def __init__(self, in_c, out_c, width, act_layer=nn.GELU, init_values=1.0, drop=0.0, drop_path=0.0, norm=True):
        super().__init__()
...
class PoissonNetNorm(nn.Module):
    ...
    def forward(self, x, mass=None):
        ...
        if mass is not None and self.mode == 'function': 
            # (B,1,1)：避免 (B,1,C)/(B,1) 被广播成 (B,B,C)（PyTorch 对不齐维数时左补 1）
            mass_sum = mass.sum(dim=1, keepdim=True).unsqueeze(-1)
            mass = mass.unsqueeze(-1)  # (B, V, 1)

            # Channel-wise mean / variance (B, 1, C)
            u = (mass * x).sum(dim=1, keepdim=True) / (mass_sum + 1e-12)
            s = (mass * (x - u)**2).sum(dim=1, keepdim=True) / (mass_sum + 1e-12)
        else:
