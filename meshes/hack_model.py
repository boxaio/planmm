...
hack_data_dir = Path(__file__).parent.parent/"data/hack_data"

bones = json.load(open(hack_data_dir/"bones_neutral.json"))
bone_names = list(bones.keys())

obj_template = read_obj(hack_data_dir/"000_generic_neutral_mesh_newuv.obj")
...
def uv1d_construct_delta(uv1d, tau):
    ...
    output = torch.nn.functional.grid_sample(
        uv1d.expand(grid.shape[0], -1, -1, -1), grid, mode='bilinear', padding_mode="border", align_corners=True,
    )
...
class HACK(nn.Module):
    def __init__(self):
        ...
        W = torch.tensor(np.load(hack_data_dir/"weight_map_smooth.npy"), dtype=torch.float32)  # [Nb, 14062]
        ...
        L = torch.tensor(cv2.imread(
            str(hack_data_dir/"Lc_mid.png"), cv2.IMREAD_GRAYSCALE)/255, dtype=torch.float32)[None, None]  # [1, 1, 256, 256]
        self.register_buffer("L", L, persistent=False)

        ts = torch.tensor(np.load(hack_data_dir/"ts_larynx.npy"), dtype=torch.float32)  # [3]
        ...
        blendshapes = torch.tensor(np.load(hack_data_dir/"blendshape.npy"), dtype=torch.float32)
    ...
    def forward(self, theta, tau, alpha, bsw, T=None, P=None, E=None):
        ...
        theta: [B, Nb, 3], pose
        tau: [B, 1], larynx
        alpha: [B, 1], appearance
        bsw: [B, 55], blendshape weights

        return: [B, Nv, 3]
        """
        B = theta.shape[0]
        theta_matrix = roma.rotvec_to_rotmat(theta)  # [B, Nb, 3, 3]
        theta_matrix_zero = theta_matrix - torch.cat([theta_matrix[:, :1], 
                                                      (torch.eye(3).to(theta)[None, None]).expand(B, N_bones-1, 3, 3)], dim=1)
...
class HACKTex(nn.Module):
    def __init__(self, config):
        super(HACKTex, self).__init__()

        # tex_space = np.load(config.tex_space_path)
        # # FLAME texture
        # if 'tex_dir' in tex_space.files:
        #     mu_key = 'mean'   # [512, 512, 3]
        #     pc_key = 'tex_dir'   # [512, 512, 3, 200]
        #     n_pc = 200
        #     scale = 1
        ...
        #     n_pc = 199
        #     scale = 255.0
        # texture_mean = tex_space[mu_key].reshape(1, -1)  # [512, 1536]
        # texture_basis = tex_space[pc_key].reshape(-1, n_pc)   # [512*512*3, 200]


        n_pc = 200
        scale = 1
        texture_mean = np.zeros((512, 1536)).reshape(1, -1)  # [512, 1536]
        texture_basis = np.zeros((512*512*3, n_pc)).reshape(-1, n_pc)   # [512*512*3, 200]

        n_tex = config.n_tex_params
