...
>>> (verts1, faces1) = mesh1
    ...
    # (assuming target_normals_list is a list of tensors each (n_verts, 3), 
    # each tensor is the target per-vertex normals of a single mesh in the batch)
...
def calc_cot_laplacian_and_cholespy_solver_until_it_works(
    ...
    for attempt_i in range(max_n_attempts):
        ...
        Lpin_rowindices_np, Lpin_colindices_np = Lpin.row, Lpin.col
        ...
        try:
            ...
            L_rowindices_np, L_colindices_np = Lcoo.row, Lcoo.col
        ...
        *,
        ...
        for verts, faces in zip(verts_list, faces_list):
            ...
            if (
                ...
                and compute_igl_arap_rhs_lefts is not None  # not needed, but for typechecker
            ):
                ...
                scipycoo_rows = igl_arap_rhs_lefts_this_mesh_scipycoo.row
                scipycoo_cols = igl_arap_rhs_lefts_this_mesh_scipycoo.col
...
LocalStepProcrustesLambda = Union[
    float,
    int,
    torch.Tensor,
    Sequence[Union[float, int, torch.Tensor]],
]


def pack_local_step_procrustes_lambda(
    local_step_procrustes_lambda: LocalStepProcrustesLambda,
    verts_list: Sequence[torch.Tensor],
) -> torch.Tensor:
    """
    Pack per-vertex Procrustes lambda for ``ProcrustesPrecompute.from_meshes``.

    - scalar: same lambda on every vertex of every mesh in the batch
    - ``(n_verts_packed,)`` tensor: one lambda per packed vertex (batch concat order)
    - sequence of length ``len(verts_list)``: per-mesh scalar or ``(n_verts_i,)`` tensor
    """
    device = verts_list[0].device
    n_verts_packed = sum(len(v) for v in verts_list)
    if isinstance(local_step_procrustes_lambda, (int, float)):
        return torch.full(
            (n_verts_packed,),
            float(local_step_procrustes_lambda),
            device=device,
            dtype=torch.float32,
        )
    if isinstance(local_step_procrustes_lambda, torch.Tensor):
        lam = local_step_procrustes_lambda.to(device=device, dtype=torch.float32).reshape(-1)
        if lam.numel() != n_verts_packed:
            raise ValueError(
                f"local_step_procrustes_lambda tensor has {lam.numel()} elements, "
                f"expected {n_verts_packed} (packed vertices)"
            )
        return lam
    if len(local_step_procrustes_lambda) != len(verts_list):
        raise ValueError(
            f"local_step_procrustes_lambda sequence length {len(local_step_procrustes_lambda)} "
            f"!= number of meshes {len(verts_list)}"
        )
    parts: List[torch.Tensor] = []
    for lam_i, verts in zip(local_step_procrustes_lambda, verts_list):
        n = len(verts)
        if isinstance(lam_i, (int, float)):
            parts.append(
                torch.full((n,), float(lam_i), device=device, dtype=torch.float32)
            )
        else:
            t = lam_i.to(device=device, dtype=torch.float32).reshape(-1)
            if t.numel() != n:
                raise ValueError(
                    f"per-mesh lambda has {t.numel()} elements, expected {n} for this mesh"
                )
            parts.append(t)
    return torch.cat(parts, dim=0)


def vertex_procrustes_lambda_from_regions(
    n_verts: int,
    default_lambda: float,
    region_vertex_indices: Sequence[Sequence[int]],
    region_lambdas: Sequence[float],
    *,
    device: Optional[torch.device] = None,
    dtype: torch.dtype = torch.float32,
) -> torch.Tensor:
    """Build ``(n_verts,)`` lambda; later regions in the list overwrite earlier on overlap."""
    if len(region_vertex_indices) != len(region_lambdas):
        raise ValueError("region_vertex_indices and region_lambdas must have the same length")
    out = torch.full((n_verts,), float(default_lambda), device=device, dtype=dtype)
    for indices, lam in zip(region_vertex_indices, region_lambdas):
        if len(indices) == 0:
            continue
...
class ProcrustesPrecompute:
    ...
    def from_meshes(
        ...
        ``local_step_procrustes_lambda`` scales the target-normal weight in the per-vertex
        Procrustes step (higher => follow target normals more). May be a global scalar, a
        packed ``(n_verts_packed,)`` tensor, or one scalar/tensor per mesh; see
        ``pack_local_step_procrustes_lambda``.
        """
        ...
        lambda_packed = pack_local_step_procrustes_lambda(
            local_step_procrustes_lambda, verts_list
        )
...
def _normalize_target_normals_packed(
    target_verts_normals_packed: torch.Tensor, eps: float = 1e-8
) -> torch.Tensor:
    """(n_verts, 3) -> unit normals; non-finite or near-zero rows become (0, 0, 1)."""
    out = torch.nan_to_num(target_verts_normals_packed, nan=0.0, posinf=0.0, neginf=0.0)
    lens = torch.linalg.norm(out, dim=-1, keepdim=True)
    good = lens.squeeze(-1) > eps
    out = torch.where(good.unsqueeze(-1), out / lens.clamp_min(eps), out)
    if (~good).any():
        default = torch.tensor((0.0, 0.0, 1.0), device=out.device, dtype=out.dtype)
        out[~good] = default
    return out


def _procrustes_rotations_from_covar(covar: torch.Tensor) -> torch.Tensor:
    """
    Batch 3x3 covariance -> rotation matrices (same convention as legacy SVD path).
    Degenerate / non-finite entries fall back to identity.
    """
    device, dtype = covar.device, covar.dtype
    n = covar.shape[0]
    identity = torch.eye(3, device=device, dtype=dtype).unsqueeze(0).expand(n, -1, -1).contiguous()
    covar = torch.nan_to_num(covar, nan=0.0, posinf=0.0, neginf=0.0)
    fro = covar.abs().amax(dim=(-1, -2))
    valid = torch.isfinite(covar).all(dim=(-1, -2)) & (fro >= 1e-10)
    if not bool(valid.any()):
        return identity.transpose(-1, -2)

    idx = torch.nonzero(valid, as_tuple=False).squeeze(-1)
    cov_sub = covar[idx]
    uu, _, vvt = torch.linalg.svd(cov_sub)
    vvt = torch.stack((-vvt[:, 0], -vvt[:, 1], vvt[:, 2]), dim=1)
    rots_sub = vvt.transpose(-1, -2).bmm(uu.transpose(-1, -2))
    entries_to_flip = torch.nonzero(rots_sub.det() <= 0, as_tuple=False).flatten()
    if len(entries_to_flip) > 0:
        uumod = uu.clone()
        uumod[entries_to_flip, :, -1] *= -1
        rots_sub[entries_to_flip] = (
            vvt[entries_to_flip]
            .transpose(-1, -2)
...
def calc_rot_matrices_with_procrustes(
    ...
    """
    target_verts_normals_packed = _normalize_target_normals_packed(target_verts_normals_packed)
    curr_deformed_verts_packed = torch.nan_to_num(
        curr_deformed_verts_packed, nan=0.0, posinf=0.0, neginf=0.0
    )
    ...
    return _procrustes_rotations_from_covar(covar)
