...
class SRenderY(nn.Module):
    def __init__(self, image_size, 
        obj_filename='/media/ubuntu/xb/cv_resnet50_face-reconstruction/assets/template_bfm.obj', 
        ...
        if rasterizer_type == 'pytorch3d':
            ...
            if 'bfm' in obj_filename:
                mesh = read_obj(obj_filename)
                uvcoords = np.load('/media/ubuntu/xb/cv_resnet50_face-reconstruction/assets/bfm_uvs2.npy')[None, ...]
                ...
                from utils.mesh import read_obj as read_obj2
                mesh = read_obj2(obj_filename)
                verts = torch.from_numpy(mesh.vs)
                uvfaces = torch.from_numpy(mesh.fvts)[None, ...]
                faces = torch.from_numpy(mesh.fvs)[None, ...]
                uvcoords = torch.from_numpy(mesh.vts)[None, ...]
        ...
        face_uvcoords = deca_util.face_vertices(uvcoords, uvfaces)  # [B, F, 3, 3]
        ...
        face_colors = deca_util.face_vertices(colors, faces)  # [B, F, 3, 3]
    ...
    def forward(self, vertices, transformed_vertices, albedos, lights=None, light_type='point'):
        ...
        normals = deca_util.vertex_normals(vertices, self.faces.expand(batch_size, -1, -1))
        face_normals = deca_util.face_vertices(normals, self.faces.expand(batch_size, -1, -1))
        transformed_normals = deca_util.vertex_normals(transformed_vertices, self.faces.expand(batch_size, -1, -1))
        transformed_face_normals = deca_util.face_vertices(transformed_normals, self.faces.expand(batch_size, -1, -1))
