...
def PtsToMesh(
    sample_verts, reference_verts, reference_faces, reference_template_or_sampler, 
    rho=lambda x : x, normalize=True, signed=False,
):
    ...
    def compute_dr_wrt(self, wrt):
        ...
        if wrt is self.reference_verts:
            r, Dr_ref, Dr_sample = sample2meshdist.squared_distance(
                self.nearest_tri, self.nearest_part, self.reference_faces,
                self.reference_verts.r.reshape((-1, 3)), self.sample_points, 
                compute_dref=True, compute_dsample=False,
            )
            result = Dr_ref
            ...
            r, Dr_ref, Dr_sample = sample2meshdist.squared_distance(
                self.nearest_tri, self.nearest_part, self.reference_faces,
                self.reference_verts.r.reshape((-1, 3)), self.sample_points, 
                compute_dref=False, compute_dsample=True, dsample_pattern=self.dsample_pattern,
            )
...
class _AabbTree(object):
    ...
    def __init__(self, v, f):
        ...
        self.cpp_handle = spatialsearch.aabbtree_compute(
            v.astype(np.float64).copy(order='C'), 
