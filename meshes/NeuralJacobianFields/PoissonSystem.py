...
def poisson_system_matrices_from_mesh(V, F, dim=3, ttype=torch.float64, is_sparse=True, cpuonly=False):
    ...
    :param V: numpy array, [V, 3]
    :param F numpy array, [F, 3]
    ...
    grad = igl.grad(vertices, faces)  # [3*F, V]
    mass = _get_mass_matrix(vertices, faces, is_sparse)  # [3*F, 3*F]
    ## TODO 2D Case ##
    if dim == 2:
        grad = grad[:-grad.shape[0]//3, :]
        mass = mass[:-mass.shape[0]//3, :-mass.shape[0]//3]

    laplace = grad.T @ mass @ grad  # [V, V]
    laplace = laplace[1:, 1:]

    rhs = grad.T @ mass   # [V, 3*F]
    b1, b2, _ = igl.local_basis(V, F)  # [F, 3]
    w = np.stack((b1, b2), axis=-1)   # [F, 3, 2]
    # print(time.time() - s)
