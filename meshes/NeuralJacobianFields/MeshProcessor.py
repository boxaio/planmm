...
class MeshProcessor:
    ...
    def meshprocessor_from_array(vertices, faces, source_dir, ttype, cpuonly=False, load_wks_samples=False, load_wks_centroids=False):
        return MeshProcessor(
                vertices, faces, ttype,source_dir, 
                cpuonly=cpuonly, load_wks_samples=load_wks_samples, 
    ...
    def compute_differential_operators(self):
        '''
        process the given mesh, compute the operators and update the ** diff_ops ** object
        '''
        ...
        self.diff_ops.frames = poisson_sys_mat.w   # [F, 3, 2], local basis
    ...
    def prepare_differential_operators_for_use(self, ttype):
        ...
        if self.diff_ops.poisson_sys_mat is None:   # not created if loaded from disk the diff ops
            diff_ops.poisson_sys_mat = PoissonSystemMatrices(
                V=self.vertices, 
                F=self.faces, 
                grad=diff_ops.grad, 
                rhs=diff_ops.rhs, 
...
def sample_points(V, F, n):
    '''
    samples n points on the given mesh, along with normals and wks. 
    Also return WKS of original faces (by averaging wks of 3 vertices of each face)
    :return:
    '''
    newF = F
    newV = V
...
class WaveKernelSignature:
    ...
    '''
    def __init__(self, vertices, faces, top_k_eig=200, timestamps=WKS_DIM):
