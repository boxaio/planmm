...
def bilateral_normal_integration(
    ...
    :param K: (Optional) A 3x3 camera intrinsic matrix, used for perspective camera models. 
                If not provided, the algorithm assumes an orthographic camera model.
    ...
    for i in pbar:
        ...
        if depth_map is None:
            ...
            z = _pcg_jacobi(
                _A_mat_ref.data, _A_mat_ref.indices, _A_mat_ref.indptr,
            ...
            A_csr = A_mat.tocsr()
            z = _pcg_jacobi(
                A_csr.data, A_csr.indices, A_csr.indptr, b_vec, d_inv, z, cg_max_iter, cg_tol,
            )
            A1z = A1 @ z
            A2z = A2 @ z
            A3z = A3 @ z
...
_build_C_csr(
    np.array([1.0, 0.0, 0.0, 1.0], dtype=np.float64), _wup_i32, _wup_ip32,
    np.array([0, 1], dtype=np.int32), np.array([0, 1, 2], dtype=np.int32), 
    2, 2,
)
_pcg_jacobi(
